...
split_data <- split(data$Productivity_Ratio, data$Community)
...
residual_model <- gam(Productivity_Ratio ~ Product+Environment+InteractionA+InteractionB, data = data)
...
kruskal_result <- kruskal.test(Residuals ~ Community, data = data)

# Print Kruskal-Wallis test result
print(kruskal_result)

# If significant, perform a Dunn test for pairwise comparisons
dunn_result <- dunn.test(data$Residuals, data$Community, method = "BH")
...
# Write the results
write_xlsx(as.data.frame(dunn_df), "Comm_PrdtRatio_StatTest.xlsx")
